...
CMD ["sh", "-c", "pnpx prisma generate && \
    pnpx prisma migrate dev --name init --schema prisma/master.prisma && \
    pnpx prisma migrate dev --name init --schema prisma/slave.prisma && \
    nest start --watch"]
